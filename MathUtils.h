...
// Distância ao quadrado entre dois pontos (sem sqrt — mais barato que calcularDistancia).
...
// Colisão AABB no plano XZ: testa sobreposição de dois quadrados de lado (raio*2).
...
// Vetor unitário no plano XZ de origem até alvo. Retorna zero se coincidentes.
...
// Gira o vetor (dx, dz) no plano XZ por `angulo` radianos. Usado por Cone/Ring/Spiral.
...
// Menor distância no plano XZ de um ponto a um segmento (base, dir unitária, comprimento).
// dir deve ser unitária no plano XZ. Usado por Beam e Wall para hitbox de faixa.
...
// True se ponto está dentro do setor de anel (raioInterno..raioExterno, ±meiaAbertura ao redor de anguloCentral).
